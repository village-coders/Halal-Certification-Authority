...
function Certificate() {
  ...
  const handleDownloadCertificate = async (certificate, specificPath = null) => {
    try {
      setDownloading(true);
      setError("");
      setSuccess("");
      ...
      // Determine the target path (prefer specificPath, then pdfPaths array, then pdfPath string)
      const targetPath = specificPath 
        || (certificate.pdfPaths && certificate.pdfPaths.length > 0 ? certificate.pdfPaths[0] : null)
        || certificate.pdfPath;

      if (targetPath) {
        // Normalize downloadUrl (ensure we hit /api/files/:id without double /api or missing /api)
        let downloadUrl = targetPath;
      ...
      // Use the backend download endpoint as fallback
      const downloadUrl = `${API_BASE_URL}/certificates/${certId}/download`;
      // Open in new tab as fallback for GridFS-stored certificates
      window.open(downloadUrl, '_blank', 'noopener,noreferrer');
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.message || "Failed to fetch certificate");
      }
      ...
      setError(err.message || "Failed to download certificate. The file may not be available yet.");
  ...
  const handleDownloadLabel = async (certificate, specificPath = null) => {
    try {
      ...
      const targetPath = specificPath 
        || (certificate.labelPaths && certificate.labelPaths.length > 0 ? certificate.labelPaths[0] : null)
        || certificate.labelPath;
                  ...
                  {filteredCertificates.map((cert) => {
                    ...
                    return (
                      <tr key={cert._id}>
                        ...
                        <td>{cert.branchId?.branchName || "N/A"}</td>
          ...
          <div className="modal modal-large">
            <div className="modal-content">
              ...
              <div style={{ padding: '24px' }}>
                ...
                {/* Certificate Documents - Support for both old pdfPath and new pdfPaths array */}
                <div className="additional-docs" style={{ marginTop: '20px', padding: '16px', background: '#f3f4f6', borderRadius: '8px', marginBottom: '24px' }}>
                  {(selectedCertificate.pdfPaths && selectedCertificate.pdfPaths.length > 0) ? (
                    selectedCertificate.pdfPaths.map((path, index) => (
                      <div key={index} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: index !== selectedCertificate.pdfPaths.length - 1 ? '16px' : 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <div style={{ background: '#e5e7eb', padding: '8px', borderRadius: '6px' }}>
                            <i className="fas fa-file-pdf" style={{ color: '#ef4444' }}></i>
                          </div>
                          <div>
                            <h4 style={{ margin: 0, fontSize: '14px', color: '#111827' }}>Certificate Document {selectedCertificate.pdfPaths.length > 1 ? index + 1 : ''}</h4>
                            <p style={{ margin: 0, fontSize: '12px', color: '#6b7280' }}>Official certification file</p>
                          </div>
                        </div>
                        <button 
                          onClick={() => handleDownloadCertificate(selectedCertificate, path)}
                          disabled={downloading}
                    ...
                    /* Fallback for single pdfPath string */
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                          <h4 style={{ margin: 0, fontSize: '14px', color: '#111827' }}>Certificate Document</h4>
                          <p style={{ margin: 0, fontSize: '12px', color: '#6b7280' }}>Official certification file</p>
                        </div>
                      </div>
                      <button 
